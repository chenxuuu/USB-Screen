...
extern UINT8C DevReport[] = {
	...
	0x10,0x01,	// USB协议版本,低位在前; 如USB2.0=0x0200，USB1.1=0x0110等。
...
// 配置描述符：配置描述符定义了设备的配置信息，一个设备可以有多个配置描述符
	...
	0x00,
	0xFF,		// 端点的报告周期．批量传送和控制传输的端点忽略为0;同步传输的端点必须为1;中断传输的端点为1-255(单位ms)
	...
	0x02,		// 端点属性．Bit7-2保留，Bit1-0<0控制:1同步:2批量:3中断>.
	EP3SIZE,	// 端点最大通信包长度(数据低位在前: 0x0040,单位Byte);
	0x00,
	0x00 		// 端点的报告周期．批量传送和控制传输的端点忽略为0;同步传输的端点必须为1;中断传输的端点为1-255(单位ms)	
};
...
// CDC初始配置参数
UINT8C LineCoding[7] = {
	0x00,0x00,0x00,0x06,	// 波特率: 12M (数据低位在前0x06000000)
