/********************************** (C) COPYRIGHT ******************************
* File Name		: UsbReports.h
* Author		: Antecer
* Version		: V1.0
* Date			: 2020年10月04日
* Description	: USB设备报告描述符
*******************************************************************************/
...
#define ProductId	0x2324				// 产品标识(PID)[对应IAP这里为0x10FD]
#define ReleaseNo	0x0001				// 发行版本(*.*.*.*)
#define LSB(x)		(x & 0xFF)
...
// 配置描述符：配置描述符定义了设备的配置信息，一个设备可以有多个配置描述符
	...
	0x89,	// 配置描述符总长度(低8位); [注:这里已经在UsbManager里面进行了自动计算]
	0x00,	// 配置描述符总长度(高8位)
	0x04,	// 配置所支持的“接口数目”．也表示该配置下接口描述符数量．
	0x01,	// 这个配置的索引值
...
// 端点描述符：USB设备中的每个端点都有自己的端点描述符，由接口描述符“端点数目”决定其数量
	...
	0x81,	// 端点地址(这里为输入端点)．Bit7<1输入:0输出>，Bit6-4保留，Bit3-0端点号．
...
// 端点描述符：USB设备中的每个端点都有自己的端点描述符，由接口描述符“端点数目”决定其数量
	0x07,	// 端点描述符大小．固定为0x07．
	0x05,	// 端点描述符类型．固定为0x05．
	0x01,	// 端点地址(这里为输出端点)．Bit7<1输入:0输出>，Bit6-4保留，Bit3-0端点号．
	0x03,	// 端点属性．Bit7-2保留，Bit1-0<0控制:1同步:2批量:3中断>.
	0x40,	// Max Packet Size LSB 端点最大通信包长度(单位byte)
	0x00,	// Max Packet Size MSB
	0x01,	// 端点的报告周期．批量传送和控制传输的端点忽略为0;同步传输的端点必须为1;中断传输的端点为1-255(单位ms)
...
// 端点描述符：USB设备中的每个端点都有自己的端点描述符，由接口描述符“端点数目”决定其数量
	0x07,	// 端点描述符大小．固定为0x07．
	0x05,	// 端点描述符类型．固定为0x05．
	0x82,	// 端点地址(这里为输入端点)．Bit7<1输入:0输出>，Bit6-4保留，Bit3-0端点号．
	0x03,	// 端点属性．Bit7-2保留，Bit1-0<0控制:1同步:2批量:3中断>.
	0x40,	// Max Packet Size LSB 端点最大通信包长度(单位byte)
	0x00,	// Max Packet Size MSB
	0x01,	// 端点的报告周期．批量传送和控制传输的端点忽略为0;同步传输的端点必须为1;中断传输的端点为1-255(单位ms)．
// 端点描述符：USB设备中的每个端点都有自己的端点描述符，由接口描述符“端点数目”决定其数量
	0x07,	// 端点描述符大小．固定为0x07．
	0x05,	// 端点描述符类型．固定为0x05．
	0x02,	// 端点地址(这里为输出端点)．Bit7<1输入:0输出>，Bit6-4保留，Bit3-0端点号．
	0x03,	// 端点属性．Bit7-2保留，Bit1-0<0控制:1同步:2批量:3中断>.
	0x40,	// Max Packet Size LSB 端点最大通信包长度(单位byte)
	0x00,	// Max Packet Size MSB
	0x01,	// 端点的报告周期．批量传送和控制传输的端点忽略为0;同步传输的端点必须为1;中断传输的端点为1-255(单位ms)
...
// 端点描述符：USB设备中的每个端点都有自己的端点描述符，由接口描述符“端点数目”决定其数量
	0x07,	// 端点描述符大小．固定为0x07．
	0x05,	// 端点描述符类型．固定为0x05．
	0x83,	// 端点地址(这里为输入端点)．Bit7<1输入:0输出>，Bit6-4保留，Bit3-0端点号．
	0x03,	// 端点属性．Bit7-2保留，Bit1-0<0控制:1同步:2批量:3中断>.
	0x40,	// Max Packet Size LSB 端点最大通信包长度(单位byte)
	0x00,	// Max Packet Size MSB
	0x01,	// 端点的报告周期．批量传送和控制传输的端点忽略为0;同步传输的端点必须为1;中断传输的端点为1-255(单位ms)．
// 端点描述符：USB设备中的每个端点都有自己的端点描述符，由接口描述符“端点数目”决定其数量
	0x07,	// 端点描述符大小．固定为0x07．
	0x05,	// 端点描述符类型．固定为0x05．
	0x03,	// 端点地址(这里为输出端点)．Bit7<1输入:0输出>，Bit6-4保留，Bit3-0端点号．
	0x03,	// 端点属性．Bit7-2保留，Bit1-0<0控制:1同步:2批量:3中断>.
	0x40,	// Max Packet Size LSB 端点最大通信包长度(单位byte)
	0x00,	// Max Packet Size MSB
	0x01,	// 端点的报告周期．批量传送和控制传输的端点忽略为0;同步传输的端点必须为1;中断传输的端点为1-255(单位ms)
...
UINT8C	DevReport[] = {
	...
	0x00,	// USB规格发布号LSB低位; 表示了本设备能适用于哪种协议，
	0x02,	// USB规格发布号MSB高位; 如USB2.0=0x0200，USB1.1=0x0110等。
...
{'U', 's', 'b', 'S', 'c', 'r', 'e', 'e', 'n'}};
// 产品名称
const struct { UINT8 bLength; UINT8 bDscType; UINT16 string[9]; } code HidStrProduct = { sizeof(HidStrProduct),0x03,
{'H', 'i', 'd', 'S', 'c', 'r', 'e', 'e', 'n'}};
// 将所有字符串描述符纳入一个指针数组
